    ...
    async def auth(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await update.message.reply_text(
                ...
                "💻 **Telegram Desktop** - надежный способ (рекомендуется)\n\n"
                "💡 **Для мобильных пользователей:**\n"
                "Если код не приходит, установите Telegram Desktop на компьютер и используйте импорт сессии.",
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "auth_method_phone":
            ...
            await query.edit_message_text(
                ...
                "**Инструкция:**\n"
                "1. Установите Telegram Desktop на компьютер:\n"
                "   • https://desktop.telegram.org/\n"
                "2. Авторизуйтесь там (используйте тот же номер)\n"
                "3. Вернитесь к боту и отправьте: /import_session\n"
                "4. Бот найдет сессию автоматически\n\n"
                "💡 **Для мобильных пользователей:**\n"
                "Установите Telegram Desktop на компьютер (даже временно) - это нужно сделать только один раз!\n\n"
    ...
    async def import_session(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if not possible_locations:
                await update.message.reply_text(
                    "❌ Telegram Desktop не найден на этом устройстве.\n\n"
                    "**Для мобильных пользователей:**\n"
                    "📱 К сожалению, сессии мобильных приложений недоступны напрямую.\n\n"
                    "**Решение:**\n"
                    "1. Установите Telegram Desktop на компьютер:\n"
                    ...
                    "2. Авторизуйтесь там (используйте тот же номер)\n"
                    "3. Вернитесь к боту и используйте /import_session\n\n"
                    "💡 Это нужно сделать только один раз - сессия сохранится!\n\n"
                    "Или попробуйте авторизацию через номер телефона: /auth"
                )
                return
