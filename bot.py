...
- Случайные задержки между запросами
    ...
    async def auth(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            # Предупреждение о проблеме с кодом
            await update.message.reply_text(
                "⚠️ **Важно:** Если код не приходит, это известная проблема.\n\n"
                "**Альтернативное решение:**\n"
                "1. Установите Telethon: `pip install telethon`\n"
                "2. Запустите в терминале:\n"
                "```python\n"
                "from telethon import TelegramClient\n"
                f"client = TelegramClient('my_session', {settings.telegram_api_id}, '{settings.telegram_api_hash}')\n"
                "await client.start()\n"
                "```\n"
                "3. Это создаст файл `my_session.session`\n"
                "4. Скопируйте его в `sessions/user_{ваш_id}.session`\n\n"
                "Или попробуйте авторизацию через номер:\n"
